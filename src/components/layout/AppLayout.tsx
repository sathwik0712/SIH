...
import { ApiGatewayConsole } from '../common/ApiGatewayConsole';
    ...
    <div className="min-h-screen flex flex-col bg-slate-100">
      <GovernmentHeader onToggleMobileMenu={() => setIsMobileMenuOpen(prev => !prev)}>
        <div className="flex items-center space-x-2">
          {/* National API Gateway Console Trigger */}
          <button
            onClick={() => setIsApiConsoleOpen(true)}
            id="open-api-console-btn"
            className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-amber-300 bg-amber-950/60 border border-amber-500/40 rounded hover:bg-amber-900/80 transition-colors shadow-sm"
            title="Open National Enterprise API Gateway Console"
          >
          ...
          <div className="fixed inset-0 z-50 md:hidden flex">
            ...
            <div className="relative w-72 max-w-[80vw] bg-slate-900 text-slate-300 flex flex-col h-full shadow-2xl z-10 border-r border-slate-800">
              ...
              <div className="p-3 bg-slate-950 border-t border-slate-800 text-[11px] flex justify-between items-center text-slate-400">
                ...
                <span className="text-[10px] font-mono text-amber-400">v2.4-Gov</span>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Enterprise Government API Gateway Console Modal */}
      <ApiGatewayConsole 
